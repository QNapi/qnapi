...
const GeneralConfig ConfigReader::readGeneralConfig(const QSettings & settings) const
{
    ...
    auto cfg = GeneralConfig(settings.value("qnapi/ui_language", "").toString(),
                             ...
                             static_cast<DownloadPolicy>(settings.value("qnapi/download_policy", DP_SHOW_LIST_IF_NEEDED).toInt()),
...
const PostProcessingConfig ConfigReader::readPostProcessingConfig(const QSettings & settings) const
{
    return PostProcessingConfig(settings.value("qnapi/post_processing", true).toBool(),
                                static_cast<EncodingChangeMethod>(settings.value("qnapi/encoding_method", ECM_CHANGE).toInt()),
...
const GeneralConfig ConfigReader::resolveP7zipPath(const GeneralConfig & config) const
{
...
const GeneralConfig ConfigReader::resolveTmpPath(const GeneralConfig & config) const
{

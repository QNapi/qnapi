...
void ConfigWriter::writeGeneralConfig(const GeneralConfig & generalConfig, QSettings & settings) const
{
    ...
    settings.setValue("qnapi/change_permissions", generalConfig.changePermissionsEnabled());
...
void ConfigWriter::writeEngineConfig(QString engineName, const EngineConfig & engineConfig, QSettings & settings) const
{
    settings.setValue(engineName + "/nick", engineConfig.nick());
    settings.setValue(engineName + "/password", engineConfig.password());
}

void ConfigWriter::writePostProcessingConfig(const PostProcessingConfig & postProcessingConfig, QSettings & settings) const
{
    settings.setValue("qnapi/post_processing", postProcessingConfig.enabled());
    settings.setValue("qnapi/encoding_method", postProcessingConfig.encodingChangeMethod());
    settings.setValue("qnapi/enc_from", postProcessingConfig.encodingFrom());
    settings.setValue("qnapi/auto_detect_encoding", postProcessingConfig.encodingAutoDetectFrom());
    settings.setValue("qnapi/enc_to", postProcessingConfig.encodingTo());
    settings.setValue("qnapi/show_all_encodings", postProcessingConfig.showAllEncodings());
    ...
    settings.setValue("qnapi/skip_convert_ads", postProcessingConfig.skipConvertAds());
    settings.setValue("qnapi/remove_words", postProcessingConfig.removeWords());
}

void ConfigWriter::writeScanConfig(const ScanConfig & scanConfig, QSettings & settings) const
{
    settings.setValue("scan/last_scan_dir", scanConfig.lastDir());
    settings.setValue("scan/skip_if_subtitles_exist", scanConfig.skipIfSubtitlesExist());

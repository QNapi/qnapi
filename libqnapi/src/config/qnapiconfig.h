...
public:
    ...
    const QNapiConfig setFirstrun(const bool & firstrun) const {
        return QNapiConfig(firstrun, version_, generalConfig_, enabledEngines_, enginesConfig_, postProcessingConfig_, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setVersion(const QString & version) const {
        return QNapiConfig(firstrun_, version, generalConfig_, enabledEngines_, enginesConfig_, postProcessingConfig_, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setGeneralConfig(const GeneralConfig & generalConfig) const {
        return QNapiConfig(firstrun_, version_, generalConfig, enabledEngines_, enginesConfig_, postProcessingConfig_, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setEnabledEngines(const QList<QPair<QString, bool>> & enabledEngines) const {
        return QNapiConfig(firstrun_, version_, generalConfig_, enabledEngines, enginesConfig_, postProcessingConfig_, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setEnginesConfig(const QMap<QString, EngineConfig> & enginesConfig) const {
        return QNapiConfig(firstrun_, version_, generalConfig_, enabledEngines_, enginesConfig, postProcessingConfig_, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setPostProcessingConfig(const PostProcessingConfig & postProcessingConfig) const {
        return QNapiConfig(firstrun_, version_, generalConfig_, enabledEngines_, enginesConfig_, postProcessingConfig, scanConfig_, lastOpenedDir_);
    }
    const QNapiConfig setScanConfig(const ScanConfig & scanConfig) const {
        return QNapiConfig(firstrun_, version_, generalConfig_, enabledEngines_, enginesConfig_, postProcessingConfig_, scanConfig, lastOpenedDir_);
    }
    const QNapiConfig setLastOpenedDir(const QString & lastOpenedDir) const {
        return QNapiConfig(firstrun_, version_, generalConfig_, enabledEngines_, enginesConfig_, postProcessingConfig_, scanConfig_, lastOpenedDir);
    }

...
SubtitleDownloadEnginesRegistry::createEngine(const QString & engineName, const QNapiConfig & config) const
{
    ...
    if(engineName == NapiProjektDownloadEngine::metadata.name())
    {
        const EngineConfig & engineConfig = config.enginesConfig()
            .value(NapiProjektDownloadEngine::metadata.name(), EngineConfig::empty);
    ...
    {
        const EngineConfig & engineConfig = config.enginesConfig()
            .value(OpenSubtitlesDownloadEngine::metadata.name(), EngineConfig::empty);
    ...
    {
        const EngineConfig & engineConfig = config.enginesConfig()
            .value(Napisy24DownloadEngine::metadata.name(), EngineConfig::empty);
        return QSharedPointer<SubtitleDownloadEngine>(
